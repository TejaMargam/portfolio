...
export default function Contact() {
  ...
  const inputClass = "w-full px-4 py-3 rounded-xl bg-slate-800/60 border border-slate-700/60 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500/60 focus:bg-slate-800 transition-all text-sm";

  const contactItems = [
    { icon: "fas fa-envelope", label: "Email", value: "vishwamargam@gmail.com", href: "mailto:vishwamargam@gmail.com", color: "text-blue-400" },
    { icon: "fas fa-phone", label: "Phone", value: "+91 8328401939", href: "tel:+918328401939", color: "text-emerald-400" },
    { icon: "fab fa-linkedin", label: "LinkedIn", value: "vishwa-margam-1b0479213", href: "https://www.linkedin.com/in/vishwa-margam-1b0479213", color: "text-blue-300" },
    { icon: "fas fa-map-marker-alt", label: "Location", value: "Hyderabad, India", href: undefined, color: "text-violet-400" },
  ];
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div ref={headerRef as any} className={`mb-16 reveal-item ${headerVisible ? "revealed" : ""}`}>
          ...
          <p className="text-slate-400 mt-6 max-w-xl">
            Ready to collaborate on innovative projects or discuss opportunities in full-stack development, IoT platforms, and desktop applications.
          </p>
        </div>

        <div className="grid lg:grid-cols-5 gap-10">
          {/* Left info */}
          <div ref={leftRef as any} className={`lg:col-span-2 space-y-4 reveal-item ${leftVisible ? "revealed" : ""}`}>
            {contactItems.map((item, i) => (
              ...
              >
                ...
                <div>
                  ...
                  {item.href ? (
                    <a href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" className="text-slate-200 text-sm hover:text-white transition-colors">
                      {item.value}
                    </a>
                  ) : (
          ...
          {/* Right form */}
          <div ref={rightRef as any} className={`lg:col-span-3 reveal-item ${rightVisible ? "revealed" : ""}`} style={{ transitionDelay: "100ms" }}>
            <div className="p-8 rounded-3xl border border-slate-800/80 bg-slate-900/40">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <input name="name" type="text" placeholder="Your name *" value={formData.name} onChange={(e) => setFormData(p => ({ ...p, name: e.target.value }))} className={inputClass} required />
                  <input name="email" type="email" placeholder="Your email *" value={formData.email} onChange={(e) => setFormData(p => ({ ...p, email: e.target.value }))} className={inputClass} required />
                </div>
                <input name="subject" type="text" placeholder="Subject" value={formData.subject} onChange={(e) => setFormData(p => ({ ...p, subject: e.target.value }))} className={inputClass} />
                <textarea name="message" rows={5} placeholder="Your message *" value={formData.message} onChange={(e) => setFormData(p => ({ ...p, message: e.target.value }))} className={`${inputClass} resize-none`} required />
                <button
                  type="submit"
