...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-24 bg-[#0d0d0d] relative">
      <div className="max-w-6xl mx-auto px-6">
        <div ref={headerRef as any} className={`mb-16 reveal-item ${headerVisible ? "revealed" : ""}`}>
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-neutral-100 mb-4">Experience</h2>
          <div className="w-16 h-1 bg-gradient-to-r from-red-500 to-rose-400 rounded-full" />
        </div>

        {/* Main Card */}
        <div ref={cardRef as any} className={`rounded-3xl border border-neutral-800/80 bg-neutral-900/40 overflow-hidden mb-6 reveal-item ${cardVisible ? "revealed" : ""}`}>
          <div className="p-8 border-b border-neutral-800/80 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-start gap-5">
              <div className="w-14 h-14 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center shrink-0">
                <i className="fas fa-building text-red-400 text-xl" />
              </div>
              <div>
                <h3 className="text-xl font-bold text-neutral-100">Full Stack Developer</h3>
                <p className="text-red-400 font-medium">Noki Technologies</p>
            ...
            <span className="px-5 py-2 rounded-full text-sm font-semibold bg-red-500/10 border border-red-500/20 text-red-400 whitespace-nowrap">
              Feb 2024 – Mar 2026
            </span>
          </div>
          ...
          <div className="p-8">
            <div className="grid md:grid-cols-2 gap-4">
              {visible.map((item, i) => (
                <div
                  key={i}
                  className="flex gap-3 p-4 rounded-xl bg-neutral-800/30 border border-neutral-800/50 hover:border-neutral-700/60 transition-colors"
                >
                  <div className="w-7 h-7 rounded-lg bg-red-500/10 border border-red-500/15 flex items-center justify-center shrink-0 mt-0.5">
                    <i className={`${item.icon} text-red-400 text-xs`} />
                  </div>
            ...
            {achievements.length > 6 && (
              <button
                ...
                className="mt-6 w-full py-3 rounded-xl border border-neutral-700/50 text-neutral-400 hover:text-neutral-100 hover:border-neutral-600 text-sm font-medium transition-all flex items-center justify-center gap-2"
              >
          ...
          {[
            ...
            <div key={i} className={`p-6 rounded-2xl border ${proj.border} ${proj.bg}`}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h4 className="text-neutral-100 font-semibold text-sm">{proj.title}</h4>
                  <p className="text-neutral-500 text-xs">Freelance</p>
                </div>
              </div>
              <p className="text-neutral-400 text-sm leading-relaxed mb-3">{proj.desc}</p>
              <div className="flex flex-wrap gap-1.5">
                {proj.tags.map((t, j) => (
                  <span key={j} className="px-2.5 py-1 rounded-md bg-neutral-800/60 border border-neutral-700/50 text-neutral-400 text-xs">{t}</span>
                ))}
              </div>
            </div>
          ))}
